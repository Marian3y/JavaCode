...
public class AppClock extends Applet implements Runnable {
	...
	public void run() {
		try {
			while (true) {
				...
				repaint();
				//		TODO:		#7 the clock wakes up every second (1000 milliseconds)Change50. Wake every 1/20 of a second
	...
	public void paint( Graphics g ) {
		...
		drawWedge( 2*Math.PI * minutes / 60, 						width/3, 	g, Color.pink );
		//drawHand ( 2*Math.PI * (seconds / 60.0),				 	width/2.25,	g, Color.cyan );
		...
		//				now go up to TODO: #7 and change it to 47
		...
		g.setFont(new Font("TimesRoman", Font.ITALIC|Font.BOLD, 73));
		...
		g.setColor( Color.blue);
		g.drawString( timeString, 10, height-7 );
	}
}